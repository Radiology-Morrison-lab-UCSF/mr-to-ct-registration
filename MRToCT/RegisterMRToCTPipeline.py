...
class RegisterMRToCTPipeline:
    ...
    def Run(self):
        ...
        if not os.path.exists(self.loc_transform):

            os.makedirs(self.dir_output, exist_ok=True)

            self.BiasCorrectIfApplicable()

            mrBrainMask = SkullStripper(self.mrGetter, useGPU=self.useGPU).CalcBrainmask(self.loc_mr_brainmask)
            ctBrainMask = SkullStripper(self.ctGetter, useGPU=self.useGPU).CalcBrainmask(self.loc_ct_brainmask)
